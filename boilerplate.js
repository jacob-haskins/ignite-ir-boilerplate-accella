...
async function install(context) {
  ...
  const spinner = print
    .spin(
      `using the Accella modified - ${red(
        "Infinite Red"
      )} boilerplate v2 (code name 'Andross')`
    )
    .succeed();
  ...
  try {
    ...
    if (answers["i18n"] === "react-native-i18n") {
      await system.spawn(`ignite add i18n@"~>1.0.0" ${debugFlag}`, {
        stdio: "inherit"
      });
    }
  ...
  if (!gitExists && !parameters.options["skip-git"] && system.which("git")) {
    ...
    system.run(
      `git init . && git add . && git commit -m "Initial commit." ${huskyCmd}`
    );
